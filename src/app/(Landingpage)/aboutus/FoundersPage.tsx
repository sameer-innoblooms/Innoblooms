...
export default function LeadershipTeam() {
  ...
  const ExpandableQuote = ({ description }: { description: string }) => {
    const [isExpanded, setIsExpanded] = useState(false);
    const words = description.split(' ');
    const truncatedQuote = words.slice(0, 20).join(' ');
  
    return (

      <Typography variant="body1" sx={{ flex: 1, mb: 4, mt: 2, color: "#4A4A4A" }}>
       {isExpanded ? description : truncatedQuote}
        {words.length > 100 && (
          <span
              ...
              >
                ...
                >
                  <Typography
                    ...
                    sx={{ lineHeight: 1.7 }}
                  >
                    {isMobile ?  <ExpandableQuote description={leader.description}/> : leader.description}
                  </Typography>
                </Grid>
              </Grid>
            </StyledPaper>
